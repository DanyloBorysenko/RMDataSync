...
public class MovieCharacterServiceImpl implements MovieCharacterService {
    ...
    public MovieCharacterServiceImpl(HttpClient httpClient, MovieCharacterRepository repository, MovieCharacterMapper mapper) {
    ...
    public MovieCharacterResponseDto getRandomCharacter() {
        ...
        MovieCharacter byId = repository.findById(randomId).orElseThrow(() ->
    ...
    private void saveDtoToDB(ApiResponseDto apiResponseDto) {
        ...
        Map<Long, MovieCharacter> movieCharactersFromDb = repository.findAllByExternalIdIn(externalIds).stream()
                .collect(Collectors.toMap(MovieCharacter::getExternalId, Function.identity()));
        Set<Long> existingIds = movieCharactersFromDb.keySet();
        externalIds.removeAll(existingIds);
        List<MovieCharacter> movieCharactersToSave = externalIds.stream().map(apiCharacterDtos::get)
                .map(mapper::parseApiResponseDto)
                .collect(Collectors.toList());
        repository.saveAll(movieCharactersToSave);
    }
    private void updateMovieCharacter(ApiResponseDto responseDto) {
        Map<Long, MovieCharacter> externalMovieCharactersMap = Arrays.stream(responseDto.getResults())
        ...
        repository.saveAll(movieCharactersToUpdate);
    }
}